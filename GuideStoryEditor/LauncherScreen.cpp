...
LauncherScreen::LauncherScreen() {
    ...
    m_menu.Add("몬스터 에디터");
    m_menu.Add("NPC 에디터");
    m_menu.Layout(kViewW * 0.5f, 300.0f, 360.0f, 60.0f, 18.0f); // 키운 창(kWinH)에 맞춰 수직 중앙쪽으로
}
...
void LauncherScreen::Render(platform::IRenderDevice& r) {
    r.Clear(kBg);
    ui::DrawCenteredText(r, "GuideStory 에디터", kViewW * 0.5f, 190.0f, 56.0f, kTitle);
    m_menu.Render(r);
    ui::DrawCenteredText(r, "편집할 대상을 선택하세요  ·  ESC 종료",
                         kViewW * 0.5f, kWinH - 60.0f, 20.0f, kHint);
}
